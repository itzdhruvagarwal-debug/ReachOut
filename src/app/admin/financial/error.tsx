...
export default function AdminFinancialError({
  error,
  reset,
}: Readonly<{ error: Error & { digest?: string }; reset: () => void }>) {
  useEffect(() => {
    logger.error("Admin financial error:", error);
  }, [error]);
  ...
  return (
    ...
    >
      ...
      <h2 className="text-xl font-bold text-foreground">Financial overview failed to load</h2>
      ...
      <Button
        variant="primary"
        aria-label="Retry loading admin financial overview"
        onClick={() => reset()}
