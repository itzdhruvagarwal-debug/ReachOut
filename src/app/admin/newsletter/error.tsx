...
export default function AdminNewsletterError({
  ...
  return (
    <div role="alert" aria-live="assertive" className="min-h-[50vh] flex flex-col items-center justify-center gap-4 text-center p-8 bg-card border border-border rounded-2xl max-w-lg mx-auto my-12 shadow-sm">
      ...
      <h2 className="text-xl font-bold text-foreground">Newsletter failed to load</h2>
      <p className="text-muted-foreground text-sm max-w-md">Newsletter subscriber data could not be fetched. Check connectivity and try again.</p>
      {error.digest && <code className="text-xs text-muted-foreground font-mono bg-muted px-2 py-1 rounded">Ref: {error.digest}</code>}
      <Button variant="primary" aria-label="Retry loading newsletter" onClick={() => reset()} className="mt-2 font-bold">Try again</Button>
    </div>
  );
}
